...
function getStore(): SqliteRegistryStore {
...
program
  .command('serve')
  .description('Start the HalluCase API server')
  .option('-p, --port <port>', 'Port number', process.env.PORT ?? '3457')
  .option('--db <path>', 'Database path', './hallucase.db')
  .action(async (options) => {
    const { createApp } = await import('./api/server.js')
    process.env.DATABASE_PATH = options.db
